...
class OrderDisplayShort {
        ...
        return {
            ...
            customer: {
                ...
                observation: clientData.observation || orderInfo.observation ||
                    "",
                delivery_option: clientData.delivery_option ||
                    clientData.deliveryOption || orderInfo.delivery_option ||
                    orderInfo.deliveryOption || "entrega",
                payment_method: clientData.payment_method ||
                    clientData.paymentMethod || orderInfo.payment_method ||
                    orderInfo.paymentMethod || "pix",
            },

            // Dados do pedido
            order: {
                order_id: orderInfo.order_id || orderInfo.id ||
                    "JD" + Date.now().toString().slice(-8),
                id: orderInfo.id || orderInfo.order_id || "",
                total: parseFloat(
                    orderInfo.total || orderInfo.total_amount || 0,
                ),
                subtotal: parseFloat(
                    orderInfo.subtotal || orderInfo.total_amount || 0,
                ),
                delivery_fee: parseFloat(
                    orderInfo.delivery_fee || orderInfo.deliveryFee || 0,
                ),
                ...
                created_at: orderInfo.created_at || orderInfo.createdAt ||
    ...
    async fetchOrderFromDatabase(orderId) {
        try {
            ...
            const response = await fetch(
                `${this.apiBase}/get-order/${orderId}`,
            );
    ...
    async getOrderFromFullURL() {
        ...
        try {
            ...
            console.error(
                "❌ Erro ao processar pedido da URL completa:",
                error,
            );
    ...
    displayCustomerInfo(customer, order) {
        ...
        const paymentMethod = this.formatPaymentMethod(
            customer.payment_method || order.payment_method,
        );
        const deliveryOption =
            (customer.delivery_option || order.delivery_option) === "retirada"
                ? "🛵 Retirada na Loja"
                : "🚗 Entrega (Delivery)";
        ...
        if (order.observation && order.observation.trim() !== "") {
            ...
            <div class="info-item">
                ...
                this.escapeHtml(order.observation)
            }</span>
            </div>
            `;
        }
        ...
        if (
            (customer.delivery_option || order.delivery_option) === "entrega" &&
            customer.address
        ) {
        ...
        if (items && items.length > 0) {
            items.forEach((item) => {
                ...
                itemsHTML += `
                    ...
                    this.escapeHtml(item.product_name || item.name)
                }</span>
    ...
    setupActionButtons(customer, order) {
        ...
        const isDelivery =
            (customer.delivery_option || order.delivery_option) === "entrega" &&
            customer.address && customer.address !== "Retirada na Loja";
        const mapsUrl = isDelivery
            ? this.createGoogleMapsUrl(customer.address)
            : null;
        const uberUrl = isDelivery
            ? this.createUberUrl(customer.address)
            : null;
    ...
    createWhatsAppUrl(phone) {
        ...
        const message = encodeURIComponent(
            "Olá, gostaria de falar sobre meu pedido!",
        );
    ...
    formatPhone(phone) {
        ...
        if (cleanPhone.startsWith("55") && cleanPhone.length === 13) {
            ...
            return `(${cleanLocal.substring(0, 2)}) ${
                cleanLocal.substring(2, 7)
            }-${cleanLocal.substring(7)}`;
        }
...
async function downloadImage() {
    ...
    try {
        ...
        const imageBlob = await ImageGenerator.generateOrderImage(
            window.orderDisplay.orderData,
        );
        ...
        await downloadDirect(
            imageBlob,
            window.orderDisplay.orderData.order.order_id,
        );
