...
const Cart = {
    ...
    initialize() {
        if (this._initialized) {
            console.warn("Carrinho já inicializado, ignorando...");
            return;
        }
    ...
    _setupAllEventListeners() {
        if (this._listenersSetup) {
            console.warn(" Listeners já configurados, ignorando...");
            return;
        }
    ...
    _setupPreviousOrdersListeners() {
        ...
        const previousOrdersList = document.getElementById(
            "previousOrdersList",
        );
    ...
    async _loadPreviousOrders() {
        try {
            ...
            let phone = localStorage.getItem("clientePhone") ||
            ...
            const phoneForDatabase = phone.startsWith("55")
                ? phone
                : `55${phone}`;

            console.log(
                ` Buscando pedidos para telefone: ${phoneForDatabase}`,
            );
            ...
            try {
                ...
                const apiResponse = await apiClient.getRecentOrdersByPhone(
                    phoneForDatabase,
                );
                ...
                if (apiResponse && apiResponse.success && apiResponse.orders) {
                    ...
                    console.log(
                        ` ${orders.length} pedidos encontrados via API.`,
                    );
                } else {
                    console.log(
                        ` API não retornou pedidos, tentando localStorage...`,
                    );
            ...
            console.log(
                `📊 Pedidos que serão exibidos: ${this._previousOrders.length}`,
            );
    ...
    _showNoPreviousOrders() {
        ...
        `;
            ...
            // Atualiza botão
            const previousOrdersBtn = document.getElementById(
                "previousOrdersBtn",
            );
    ...
    _getOrdersByPhoneFromLocalStorage(phone) {
        console.log(
            `🔍 Buscando pedidos no localStorage para telefone: ${phone}`,
        );
        ...
        orders.sort((a, b) => {
            const dateA = new Date(
                a.order?.created_at || a.order?.timestamp || 0,
            );
            const dateB = new Date(
                b.order?.created_at || b.order?.timestamp || 0,
            );
            return dateB - dateA;
        });
    ...
    _normalizePhone(phone) {
        ...
        const withCountryCode = cleanPhone.startsWith("55")
            ? cleanPhone
            : `55${cleanPhone}`;
        ...
        return {
            ...
            without55: cleanPhone.startsWith("55")
    ...
    _loadFromLocalStorage() {
        ...
        console.log(
            `📜 ${this._previousOrders.length} pedidos anteriores carregados do localStorage.`,
        );
    ...
    _renderPreviousOrders() {
        ...
        ordersToShow.forEach((order, index) => {
            console.log(
                `📄 Renderizando pedido ${index + 1} de ${ordersToShow.length}`,
            );
        ...
        const previousOrdersBtn = document.getElementById("previousOrdersBtn");
        if (previousOrdersBtn) {
            previousOrdersBtn.innerHTML =
                `📋 Pedidos anteriores (${ordersToShow.length})`;
        }
    },
    _createOrderHtml(order) {
        ...
        const apiOrderId = order.order?.order_id || order.order?.id ||
            order.order_id;
        const displayOrderId = order.order?.order_id || order.order?.id ||
            order.order_id || shortId;

        console.log(
            `🔑 IDs disponíveis: shortId=${shortId}, apiOrderId=${apiOrderId}, displayOrderId=${displayOrderId}`,
        );
            ...
            for (const key of keys) {
                if (key.startsWith("order_")) {
                    try {
                        ...
                        const storedApiId = storedOrder.order?.order_id ||
                            storedOrder.order?.id;
                        const storedDisplayId = storedOrder.order?.order_id ||
                            storedOrder.order?.id || storedOrder.order_id;

                        if (
                            storedDisplayId === displayOrderId ||
                            storedApiId === apiOrderId
                        ) {
                            actionOrderId = key.replace("order_", "");
                            isLocalStorageOrder = true;
                            console.log(
                                `🔍 Encontrou correspondência no localStorage: ${actionOrderId}`,
                            );
                            break;
                        }
                    } catch (e) {
        ...
        // Formata a data
        const orderDate = order.created_at || order.order?.created_at ||
            order.order?.timestamp || order.date || order.timestamp;
        const formattedDate = this._formatOrderDate(orderDate);

        // Calcula o total
        const total = order.total || order.order?.total || order.total_amount ||
        ...
        if (orderItems && orderItems.length > 0) {
            ...
            productsList =
                '<div class="order-item-detail"><span>Clique em "Ver Detalhes" para ver os itens</span></div>';
        }

        // Gera o link de detalhes
        // Se tivermos um ID oficial (que começa com JD-), usamos apenas orderId
        const isOfficialId = actionOrderId &&
            actionOrderId.toString().startsWith("JD-");
        const detailsLink = `/order.html?orderId=${
            encodeURIComponent(actionOrderId)
        }${(isLocalStorageOrder && !isOfficialId) ? "&source=local" : ""}`;
    ...
    async repeatOrder(orderId) {
        ...
        try {
            console.log(
                `Buscando detalhes do pedido ${orderId} para repetição...`,
            );
            ...
            try {
                ...
                if (
                    response && response.success !== false && response.orderData
                ) {
                    orderDetails = response.orderData;
                    source = "api";
                    console.log(`Pedido ${orderId} encontrado na API`);
                } else {
                    console.log(`API retornou sem dados para ${orderId}`);
                    throw new Error("API sem dados");
                }
            } catch (apiError) {
                console.log(
                    "Não conseguiu buscar da API, tentando localStorage...",
                );
                ...
                if (orderDetails) {
                    source = "localStorage";
                    console.log(
                        `Pedido ${orderId} encontrado no localStorage`,
                    );
                } else {
                        ...
                        for (const key of orderKeys) {
                            try {
                                ...
                                if (storedApiId === orderId) {
                                    ...
                                    console.log(
                                        `Pedido encontrado no localStorage pelo ID da API: ${key}`,
                                    );
                                    break;
                                }
                            } catch (e) {
            ...
            if (!orderDetails) {
                ...
                const localKey = orderId.startsWith("order_")
                    ? orderId
                    : `order_${orderId}`;
                ...
                try {
                    ...
                    if (storedData) {
                        ...
                        console.log(
                            `Pedido encontrado com chave direta: ${localKey}`,
                        );
                    }
                } catch (e) {
            ...
            if (!orderDetails) {
                console.warn(
                    `Pedido ${orderId} não encontrado em nenhuma fonte.`,
                );
                window.showNotification(
                    "Pedido não encontrado. Talvez tenha expirado ou sido removido.",
                    3000,
                    "error",
                );
                return;
            ...
            if (items.length === 0) {
                console.warn(` Pedido ${orderId} encontrado mas sem itens.`);
                window.showNotification(
                    "Pedido encontrado mas sem itens para repetir.",
                    3000,
                    "warning",
                );
                return;
            ...
            console.log(
                ` Fonte dos dados: ${source}, ${items.length} itens encontrados`,
            );
            ...
            items.forEach((item) => {
                ...
                if (itemName && itemPrice) {
                    ...
                    let itemImage = item.image || item.image_url ||
                        item.imagem || item.product_image;

                    console.log(` Imagem original do item: ${itemImage}`);
                    ...
                    if (
                        ...
                        const foundProduct = menuProducts.find((p) =>
                            p.id == itemId || p.name == itemName
                        );

                        if (foundProduct && foundProduct.image) {
                            itemImage = foundProduct.image;
                            console.log(
                                ` Imagem recuperada do menu global: ${itemImage}`,
                            );
                            ...
                            console.log(
                                `Usando fallback por ID: ${itemImage}`,
                            );
                        } else {
                            itemImage = "img/logos/Logo.png";
                            console.log(
                                ` Usando fallback logo: ${itemImage}`,
                            );
                        }
                    }
                    ...
                    const product = {
                        ...
                        available_days: [
                            ...
                            "sábado",
                            "domingo",
                        ], // Ignora trava de dia na repetição
                    };

                    console.log(
                        "🛒 Adicionando produto normalizado ao carrinho:",
                        product,
                    );
            ...
            if (
                previousOrdersList &&
                previousOrdersList.style.display === "block"
            ) {
            ...
            window.showNotification(
                "Erro ao repetir pedido. Tente novamente.",
                "error",
            );
        }
    },
    ...
    _findOrderInLocalStorage(orderId) {
        ...
        const orderKeys = keys.filter((key) => key.startsWith("order_"));
        console.log(
            `🔍 Verificando ${orderKeys.length} pedidos no localStorage...`,
        );
    ...
    _setupCheckoutListeners() {
        const closeCheckoutModal = document.getElementById(
            "closeCheckoutModal",
        );
    ...
    _setupFormListeners() {
        ...
        const deliveryInfoToggle = document.getElementById(
            "deliveryInfoToggle",
        );
    ...
    _togglePaymentMethods(deliveryOption) {
        ...
        const dinheiroOption = Array.from(options).find((opt) =>
            opt.value === "dinheiro"
        );
        const pixOption = Array.from(options).find((opt) =>
            opt.value === "pix"
        );
        const cartaoOption = Array.from(options).find((opt) =>
            opt.value === "cartao"
        );
    ...
    async _fetchAddressByCep(cep) {
        ...
        const safetyTimeout = setTimeout(() => {
            if (cepInput && cepInput.value === "Buscando...") {
                console.warn(
                    " Timeout na busca do CEP. Restaurando valor original.",
                );
                cepInput.value = originalValue;
                cepInput.disabled = false;
                window.showNotification(
                    "Tempo limite excedido ao buscar CEP. Tente novamente.",
                    3000,
                    "warning",
                );
            }
        ...
        try {
            ...
            window.showNotification(
                "Endereço encontrado! Agora insira o número.",
                3000,
                "success",
            );
    ...
    _fillAddressFields(addressData) {
        const fields = {
            ...
            "customerCity": `${addressData.localidade || ""} - ${
                addressData.uf || ""
            }`,
    ...
    addToCart(product) {
        const currentDay = window.getCurrentDayName
            ? window.getCurrentDayName()
            : "quarta";

        // Verificar disponibilidade
        if (
            !product.available_days ||
            !product.available_days.includes(currentDay)
        ) {
            const diaDisponivel =
            ...
            const message =
                ` ${product.name} só está disponível na(s) ${diaDisponivel}.`;
        ...
        if (existingItem) {
            ...
            if (
                (!existingItem.image ||
                    existingItem.image.includes("Logo.png")) && product.image
            ) {
    ...
    updateCartUI() {
        ...
        if (cartItemsContainer) {
            ...
            this.cartItems.forEach((item) => {
                ...
                const imageUrl = item.image || item.imagem ||
                    "img/logos/Logo.png";
                ...
                cartItem.querySelector(".delete-item-btn").addEventListener(
                    "click",
                    () => {
                ...
                setTimeout(async () => {
                    ...
                    if (savedNumber) {
                        ...
                        if (numberInput) {
                            ...
                            console.log(
                                "✅ Número pré-preenchido:",
                                savedNumber,
                            );
                        }
                    }

                    const savedComplement = localStorage.getItem(
                        "lastCustomerComplement",
                    );
                    if (savedComplement) {
    ...
    _handleCheckoutSubmit(e) {
        ...
        const name = document.getElementById("customerName")?.value.trim() ||
            "";
        const phone =
            document.getElementById("customerPhone")?.value.replace(
                /\D/g,
                "",
            ) || "";
        ...
        const paymentMethod = document.getElementById("paymentMethod")?.value ||
            "";
        const observation =
            document.getElementById("customerObservation")?.value.trim() || "";
        ...
        if (deliveryOption === "entrega") {
            ...
            if (!cep || !street || !number || !neighborhood || !city) {
                window.showNotification(
                    "Por favor, preencha todos os campos de endereço para entrega.",
                    3000,
                    "error",
                );
                this._submitting = false;
        ...
        localStorage.setItem(
            "lastCustomerPhone",
            this._formatPhoneForDisplay(phone),
        );
    ...
    async _processOrder(
        ...
        console.log(
            `📝 Criando pedido ${orderId} para ${name} (${phone}) via API...`,
        );

        // Prepara dados do endereço
        let fullAddress = "Retirada na Loja";

        const { street, number, neighborhood, city, cep, complement } =
            addressData;

        if (
            deliveryOption === "entrega" && street && number && neighborhood &&
            city && cep
        ) {
            fullAddress =
        ...
        try {
            ...
            if (apiResult && apiResult.success) {
                ...
                universalLink = this._generateUniversalOrderLink(
                    apiResult.orderId,
                );
        ...
        const shortId = this._saveToLocalStorage({
            customer: clientData,
            order: {
                order_id: apiResult && apiResult.success
                    ? apiResult.orderId
                    : orderId, // Prioriza ID da API
                id: apiResult && apiResult.success
                    ? apiResult.orderId
                    : orderId, // Campo de compatibilidade
                total: total,
                subtotal: subtotal,
        ...
        console.log(
            `📱 Telefone salvo: ${phone} (sem 55) e ${phoneWith55} (com 55)`,
        );
    ...
    _saveToLocalStorage(orderData) {
        ...
        if (this._currentUserId) {
            localStorage.setItem(
                `user_${this._currentUserId}_last_order`,
                shortId,
            );
        }

        console.log(
            `Pedido salvo localmente: ${orderKey} para usuário ${
                this._currentUserId || "desconhecido"
            }`,
        );
        return shortId;
    },
    ...
    _generateWhatsAppMessage(orderData, shortId, universalLink = null) {
        ...
        const deliveryOption = customer.delivery_option ||
        ...
        message += `\n*Modalidade:* ${
            deliveryOption === "retirada" ? "_Retirada na Loja_" : "_Entrega_"
        }\n`;
        ...
        message +=
            `_Clique no link acima para ver todos os detalhes do pedido!_\n\n`;
    ...
    loadCartFromStorage() {
        try {
            ...
            if (savedCart) {
                ...
                console.log(
                    ` Carrinho carregado: ${this.cartItems.length} itens`,
                );
            }
        } catch (error) {
